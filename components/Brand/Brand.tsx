...
export const Brand: React.FC<BrandProps> = ({
  ...
  const spacing = orientation === 'horizontal' ? currentSize.spacingHorizontal : currentSize.spacingVertical;
  ...
  const renderLogo = () => {
    if (logo) {
      return <div className={`${currentSize.logo} ${logoClassName}`}>{logo}</div>;
    }
    ...
    if (logoPath) {
      return (
        <>
          {/* Light theme logo */}
          <img
            src={isDarkModeSupported && theme === 'auto' ? '/brand/logo.png' : logoPath}
            alt={logoAlt}
            className={`${currentSize.logo} ${brandStyles.common.logo} ${logoClassName} ${
              isDarkModeSupported && theme === 'auto' ? brandStyles.logoVisibility.lightAuto : 
              theme === 'dark' ? 'hidden' : ''
            }`}
          />
    ...
    return (
      <div className={`${currentSize.logo} ${brandStyles.common.defaultLogoBackground} ${logoClassName}`}>
  ...
  const renderText = () => {
    ...
    const textColors = isDarkModeSupported && theme === 'dark' 
      ? brandStyles.textColors.dark 
      : brandStyles.textColors.light;

    return (
      <div className={brandStyles.common.textContainer}>
        <span className={`${brandStyles.common.textPrimary} ${textColors.primary} ${currentSize.text} ${textClassName}`}>
          {brandName}
        </span>
        {tagline && (
